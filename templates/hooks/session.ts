...
export async function saveSessionData(hookType: string, payload: Record<string, unknown>): Promise<void> {
  try {
    ...
    let sessionData: Array<{timestamp: string; hookType: string; payload: Record<string, unknown>}> = []
    try {
